...
from SerenaRobot import OWNER_ID
...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./SerenaRobot/resources/blackbg.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./SerenaRobot/resources/Chopsic.otf", 330)
    ...
    fname2 = "LogoBySerena.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="💕 Made By @SerenaYvonneRobot Support @PigasusSupport")
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./SerenaRobot/resources/Maghrib.ttf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
    ...
    fname2 = "LogoBySerena.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @SerenaYvonneRobot")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @PigasusSupport, {e}')
